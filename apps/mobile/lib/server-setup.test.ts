...
describe("bootstrapCommand", () => {
  ...
  it("names the release assets exactly as the release workflow publishes them", () => {
    expect(bootstrapCommand("linux")).toContain("QUIBT_RELEASE=0.2.20");
    expect(bootstrapCommand("darwin")).toContain("QUIBT_RELEASE=0.2.20");
    expect(bootstrapCommand("win32")).toContain("quibtbot-win32-x64.exe");
    expect(bootstrapCommand("win32")).toContain("checksums-0.2.20.txt");
