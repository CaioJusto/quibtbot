...
describe("previous release validation", () => {
  it("aborts when env and install state releases diverge", () => {
    ...
    saveInstallState(dataDir, initialInstallState("0.2.11", new Date()));
    writeFileSync(
      env.path,
  ...
  it("returns manual_recovery_required when rollback throws", async () => {
    ...
    const recovery = await restoreRollback(
      ...
      },
      {
        release: "0.2.11",
        images: [
          { reference: "ghcr.io/quibt/quibt-stack:0.2.11", id: "sha256:a" },
          { reference: "ghcr.io/quibt/quibt-supervisor:0.2.11", id: "sha256:b" },
          { reference: "ghcr.io/quibt/quibt-computer:0.2.11", id: "sha256:c" },
        ],
        envSnapshotPath,
