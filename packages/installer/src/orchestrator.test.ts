...
const COMPOSE_IMAGES = [
  ...
  "ghcr.io/quibt/quibt-computer:0.2.20",
  "ghcr.io/quibt/quibt-supervisor:0.2.20",
...
describe("download das imagens com progresso", () => {
  it("avisa o tamanho, puxa uma imagem por vez e emite camadas feitas/total", async () => {
    ...
    expect(progress).toContainEqual({
      image: "ghcr.io/quibt/quibt-stack:0.2.20",
      label: "quibt-stack:0.2.20",
      index: 4,
      count: 4,
      layersDone: 1,
      layersTotal: 2,
    });
    expect(messages).toContain("Baixando imagem 4 de 4: quibt-stack:0.2.20 — 1/2 camadas");
  ...
  it("na retomada baixa só o que falta e não cobra os 10 GB de novo", async () => {
    ...
    expect(result.ok).toBe(true);
    expect(pulled).toEqual(["ghcr.io/quibt/quibt-stack:0.2.20"]);
    const messages = events.map((event) => event.message);
