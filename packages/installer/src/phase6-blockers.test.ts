...
describe("state validation split", () => {
  ...
  it("updates from an old release when env matches exactly", async () => {
    ...
    expect(result.release).toBe("0.2.13");
    expect(readFileSync(env.path, "utf8")).toMatch(/QUIBT_STACK_VERSION=0\.2\.13/);
  });
});
  ...
  it("reports completed rollback with cleanup warning when runner rejects cleanup", async () => {
    ...
    const recovery = await performCompleteRollback({
      ...
      rollback: {
        release: "0.2.13",
        images: [
          { reference: "ghcr.io/quibt/quibt-stack:0.2.13", id: "sha256:id" },
          { reference: "ghcr.io/quibt/quibt-supervisor:0.2.13", id: "sha256:id" },
          { reference: "ghcr.io/quibt/quibt-computer:0.2.13", id: "sha256:id" },
        ],
        envSnapshotPath,
