  ...
  it("always removes the restore dump and reports cleanup failures without hiding success", async () => {
    ...
    const recovery = await performCompleteRollback({
      ...
      rollback: {
        release: "0.2.20",
        images: [
          { reference: "ghcr.io/quibt/quibt-stack:0.2.20", id: "sha256:id" },
          { reference: "ghcr.io/quibt/quibt-supervisor:0.2.20", id: "sha256:id" },
          { reference: "ghcr.io/quibt/quibt-computer:0.2.20", id: "sha256:id" },
        ],
        envSnapshotPath,
