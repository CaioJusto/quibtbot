...
async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), "quibt-home-"));
  dirs.push(root);
  const store = new LocalAgentHomeStore(root);
  const home = store.pathForWorkspace(context.workspaceId);
  await mkdir(home, { recursive: true });
  return { root, store, home };
}

describe("LocalAgentHomeStore path containment", () => {
  it("shares workspace files across bot identities", async () => {
    const { store } = await fixture();
    await store.writeFile("bot-1", "notes/shared.txt", "shared", context);
    expect(await store.readFile("bot-2", "notes/shared.txt", context)).toBe("shared");
  });
