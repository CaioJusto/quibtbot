...
export class ScriptedAgentRuntime implements AgentRuntime {
  ...
  async *run(request: AgentRunRequest, context: AdapterContext): AsyncIterable<AgentRuntimeEvent> {
    ...
    try {
      ...
      // Positional, not by tool name: two `shell` calls in the same script would otherwise
      // share an idempotency key and the second would be swallowed as a duplicate.
      let slot = 0;
      for (const turn of script) {
        ...
        if (turn.assistant) {
          yield { type: "progress", text: "Trabalhando…" };
        ...
        for (const call of turn.toolCalls ?? []) {
          if (call.name === "run_subagent") {
            ...
            yield {
              ...
              name: call.name,
              args: call.args,
              executionId: `${request.runId}:${call.name}#${slot}`,
            };
            yield {
              type: "subagent",
              agentId,
...
export function inferScript(
        ...
        toolCalls: [
          {
            ...
            args: { collection: "notes", title: "Quibt Bot result", body: prompt },
          },
        ],
        complete: true,
      },
  ...
  if (/\bask_bot\b/i.test(prompt) || lower.includes("ask the teammate")) {
    const name = namedTeammate(prompt) ?? "Writer";
    const message =
      prompt.replace(/^.*?(?:ask_bot|ask the teammate)\s+@?\S+\s*:?\s*/i, "").trim() || prompt;
    return [
      {
        assistant: `asking ${name} before I continue.`,
        toolCalls: [{ name: "ask_bot", args: { name, message } }],
      },
    ];
  }
  if (/\bmessage_teammate\b/i.test(prompt) || lower.includes("recado for")) {
    const name = namedTeammate(prompt) ?? "Writer";
    const message =
      prompt.replace(/^.*?(?:message_teammate|recado for)\s+@?\S+\s*:?\s*/i, "").trim() || prompt;
    return [
      {
        assistant: `sending a recado to ${name}.`,
        toolCalls: [{ name: "message_teammate", args: { name, message } }],
        complete: true,
      },
    ];
  }
    ...
    return [
      {
        ...
        memory: [{ scope: "bot", path: "MEMORY.md", content: prompt.trim() }],
        complete: true,
      },
    ];
  }
...
function namedTeammate(prompt: string) {
  return (
    /(?:ask_bot|ask the teammate|message_teammate|recado for)\s+@?([A-Za-z0-9][A-Za-z0-9_-]{0,39})/i.exec(
      prompt,
    )?.[1] ?? namedBot(prompt)
  );
}
