...
const REMOTE_GUIDE: MachineGuide = {
  ...
  youNeed: [
    ...
    "API no notebook e computador na VPS: um alias `Host` no ~/.ssh/config deste aparelho, com chave (nunca senha colada aqui). O Quibt fala com o Docker via `docker -H ssh://alias` e abre um túnel SSH temporário do noVNC até 127.0.0.1 — a tela ao vivo aparece no painel. Este caminho recusa 80/443 publicados na VPS e não publica a 7091.",
    "Ou, sem SSH: o dono daquele host liga o profile `supervisor-tls` (`docker compose --profile supervisor-tls up -d supervisor supervisor-tls`). A porta do supervisor não é publicada sozinha: quem atende é o Caddy, por https, num nome público. Cole a URL https e o token do supervisor — a mesma senha longa que está no .env do servidor.",
  ],
  steps: [
    ...
    "Se a API roda neste notebook, cole o alias SSH do ~/.ssh/config e toque em Testar (sem chave nem senha). Se o Quibt inteiro está na VPS, cole a URL https do supervisor e o token.",
...
const HETZNER_GUIDE: MachineGuide = {
  ...
  steps: [
    ...
    "Volte aqui: se a API está no notebook, cole o alias SSH; se o Quibt inteiro está na VPS, cole a URL do supervisor e o token. Teste e salve.",
...
const DIGITALOCEAN_GUIDE: MachineGuide = {
  ...
  steps: [
    ...
    "Se a API está no notebook, cole o alias SSH; se o Quibt inteiro está na VPS, cole a URL do supervisor e o token. Teste e salve.",
...
const GENERIC_VPS_GUIDE: MachineGuide = {
  ...
  steps: [
    ...
    "Se a API está no notebook, cole o alias SSH; se o Quibt inteiro está na VPS, cole a URL e o token. Teste e salve.",
