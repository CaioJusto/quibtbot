...
**Preciso assumir o controle para ver a tela?** Sim. A URL assinada chega ao soquete VNC de verdade: o `view_only` do noVNC é só JavaScript no navegador, então a API **não emite** capacidade enquanto este ator não tiver o lease. Sem o teclado, o painel mostra o placeholder e pede **Assumir controle**. **Liberar** tira a URL na hora. O controle vale 15 minutos a partir do último uso: cada tecla, clique ou heartbeat do app renova o prazo (a tela mostra "controle até HH:mm"); quem para de usar por 15 minutos devolve o computador ao bot sozinho. A capacidade sai pela API e o app serve em `/novnc/…`; ela vale para aquele servidor de tela até expirar, porque o cliente noVNC busca os próprios scripts e abre o WebSocket em caminhos diferentes.
