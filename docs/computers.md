...
**O bot pode pedir a tela?** Sim. Quando só a pessoa pode agir (login, 2FA, captcha, pagamento), o bot chama `request_takeover`: o run fica em `waiting_takeover`, o computador marca `waitingTakeover`, e cliques/teclas do agente contra essa sessão são recusados até você **Assumir controle** e depois **Liberar**. Web e celular continuam no noVNC. No app desktop há um **Navegador** extra (Chromium embutido, partição `persist:bot-<id>`), com aviso na bandeja — não é o Chrome deste Mac/Windows, não é o Chromium do Linux, e não existe no navegador comum nem no celular.
