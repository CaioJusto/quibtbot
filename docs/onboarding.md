...
**Com um assistente de IA**: o [`SETUP_PROMPT.md`](../SETUP_PROMPT.md) é um prompt pronto (PT + EN) para colar no Claude ou no ChatGPT e ser guiado por esses mesmos comandos, um passo de cada vez.
...
| CLI no host | Se a API detectar `claude`, `codex`, `grok` ou uma CLI ACP extra, escolha a CLI já conectada. Não cole chave. A CLI usa as ferramentas do computador do bot. | A assinatura usada pela CLI |
...
A CLI local roda no host dos processos da API/worker; as ferramentas dela mexem no computador
do bot, não no Mac/Windows. Veja [cli-engines.md](./cli-engines.md). Se nenhuma for encontrada,
a tela avisa e mantém as outras opções.
...
| Tela preta no computador | Mande outra mensagem, ou toque em Assumir controle. No Docker, o desktop só sobe quando o bot precisa dele. No app desktop, o botão **Navegador** é um Chromium só daquele bot — não substitui a tela Linux no celular nem no Chrome da web. |
