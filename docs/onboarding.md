...
1. Baixe o instalador em [quibt.com.br](https://quibt.com.br) ou nas [Releases do GitHub](https://github.com/CaioJusto/quibtbot/releases/latest):
   - Mac (Apple silicon): `QuibtBot.dmg` — confira o `signing-status-mac.json` da v0.2.15. Enquanto o DMG do CI ainda não tiver sido substituído pelo notarizado, o macOS avisa; na primeira vez, clique com o botão direito → Abrir. Mac Intel: ainda sem instalador — só rodando a partir do código-fonte (veja o README).
...
```bash
curl -fsSL https://raw.githubusercontent.com/CaioJusto/quibtbot/v0.2.15/scripts/install.sh \
  | QUIBT_RELEASE=0.2.15 sh
```
