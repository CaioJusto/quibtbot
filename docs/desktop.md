...
| -------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| macOS    | `v0.2.17` is a source candidate without a tag or installer. The `v0.2.14`, `v0.2.10` and `v0.2.9` DMGs are Developer ID signed, notarized and stapled. | Restore the personal-team notary credential, produce and verify the exact `v0.2.17` DMG, status and checksums, then set `DESKTOP_SIGNING.mac` to `true`. Intel (x64) is not published; Apple silicon only. |
...
| Linux    | `QuibtBot.AppImage` is published as an **unsigned test build**; the next release will also ship the x64/arm64 `quibtbot` CLI binaries for server installs.   | No store signature. Needs `libfuse2`; mark the download executable before running it; Docker Engine or Desktop is installed by the user. |
