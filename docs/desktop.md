...
- macOS Apple silicon: `QuibtBot.dmg` — check the attached `signing-status-mac.json`; if it reports an unsigned build, macOS warns the first time, **right-click → Open**
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| macOS    | The published `v0.2.19` DMG is **unsigned and not notarized** (`signing-status-mac.json` says `signed: false, notarized: false`); macOS warns the first time — **right-click → Open**. The `v0.2.14`, `v0.2.10` and `v0.2.9` DMGs are Developer ID signed, notarized and stapled. | Restore the personal-team notary credential, replace the published `v0.2.19` DMG with a signed, notarized and stapled build, update its status and checksums, then set `DESKTOP_SIGNING.mac` to `true`. Intel (x64) is not published; Apple silicon only. |
...
| Linux    | `QuibtBot.AppImage` is published as an **unsigned test build**; the release also ships the x64/arm64 `quibtbot` CLI binaries for server installs.   | No store signature. Needs `libfuse2`; mark the download executable before running it; Docker Engine or Desktop is installed by the user. |
