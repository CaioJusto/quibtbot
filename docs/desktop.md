...
2. The wizard generates secrets and runs Compose when the compose file is available (source checkout or bundled `extraResources`). Before downloading it checks for 10 GB free, says it is about to fetch ~1.7 GB (≈10 GB on disk) and shows a progress bar per image (image 2 of 4, layers done/total). A download that goes silent for 3 minutes is retried up to three times; already-downloaded layers stay cached, so re-running the wizard only fetches what is missing. You can also point `QUIBT_WEB_URL` at a remote deploy.
3. **Second launch.** When the stack is installed but stopped (after a reboot, or after quitting Docker), the app does not show the install wizard again: it opens Docker Desktop if needed, runs `docker compose up --wait` itself and shows “Ligando o Quibt Bot…” with the current step, then opens the product. If that fails, the real error and a **Tentar de novo** button appear. `quibtbot install` on a machine that is already installed does the same and prints “já instalado e no ar em <URL>”.
