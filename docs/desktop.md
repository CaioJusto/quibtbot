...
- macOS Apple silicon: `QuibtBot.dmg` — on `v0.2.15`, check the attached `signing-status-mac.json`; until the CI build is replaced by the notarized build, macOS warns the first time, **right-click → Open**
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| macOS    | The `v0.2.15` pipeline first creates an unsigned Apple-silicon `QuibtBot.dmg`. The final status is the attached `signing-status-mac.json`. The `v0.2.14`, `v0.2.10` and `v0.2.9` DMGs are Developer ID signed, notarized and stapled. | Replace the `v0.2.15` DMG, `signing-status-mac.json` and checksums with the notarized build, then set `DESKTOP_SIGNING.mac` to `true`. Intel (x64) is not published; Apple silicon only. |
...
| Linux    | `QuibtBot.AppImage` is published as an **unsigned test build**; `v0.2.15` also ships the x64/arm64 `quibtbot` CLI binaries for server installs.   | No store signature. Needs `libfuse2`; mark the download executable before running it; Docker Engine or Desktop is installed by the user. |
