...
- **Desktop provider** is disabled. Running the Electron client does not run model commands on the host. The packaged app can embed a per-bot Chromium (`persist:bot-<id>`) for the owner; that is not `SANDBOX_PROVIDER=desktop` and does not replace Docker/VPS/E2B/Box/Daytona.
