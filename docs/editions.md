...
| Computer | You pick from the catalog: Docker here, your VPS / remote supervisor, E2B, or Box (BYOK) | Quibt manages it. No machine picker |
...
3. If you own the deploy, pick the computer from the catalog (Docker, remote supervisor / VPS, E2B, or Box). Keys and the supervisor URL can be pasted in the UI.
...
- `SANDBOX_PROVIDER` is the fallback: it is used when nothing is saved or when the edition does not
  allow a choice. E2B, Box, and a remote supervisor accept keys and endpoints in **Settings → Máquina**
...
`SANDBOX_PROVIDER=e2b` or `box`. If you host "cloud" only for yourself on one box, say so out loud
...
| [computers.md](./computers.md) | Docker, VPS, E2B, Box — what the owner must do |
