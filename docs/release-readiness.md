...
## Current audit evidence (2026-08-26, `v0.2.9` → `v0.2.12`)
...
| Desktop | **`v0.2.9` / `v0.2.10`:** the Apple-silicon DMG is Developer ID signed, notarized and stapled. **`v0.2.11`:** the CI DMG is unsigned. **`v0.2.12`:** the release workflow initially publishes an unsigned CI build and records the real status in `signing-status-mac.json`. Windows and Linux remain unsigned test builds. | `v0.2.12` Mac DMG: replace it with the maintainer's notarized build, update its signing status and checksums, then flip `DESKTOP_SIGNING.mac`. Windows needs an Authenticode certificate. Intel Mac is not published. |
...
| --- | --- | --- |
| Source quality | format, lint, TypeScript, `verify:fast`, full `verify` | Green on the final tree: 308 test files passed, 2 skipped; 2,496 tests passed, 6 skipped; plus 4/4 browser journeys |
...
| Desktop | macOS Apple-silicon DMG, Windows installer, Linux AppImage; signing status attached; `DESKTOP_SIGNING` matches the attached status | `v0.2.12`: Mac DMG is the unsigned CI build until the notarized replacement is uploaded; Windows and Linux are explicitly unsigned; Intel is not part of this release |
...
| Publication | public repository, source tag equals artifact version, checksums and release notes | Publish the tag-generated assets and replace the embedded provisional CLI digests with the real v0.2.12 checksums before the production OTA |
