...
profile is also unavailable. `v0.2.18` carries the Box parity fix and is the next source candidate;
...
| Publication | `v0.2.14` is public production. `v0.2.16` has public image tags and a private GitHub draft, but no public release. `v0.2.18` is only a source candidate | Do not publish or deploy the candidate site/OTA until the release manifest has real CLI hashes and the Mac notary gate passes |
...
| Supply chain | versioned CLI + checksums; packaged Docker images pinned by digest | `v0.2.16` draft CLI sidecars and tagged images were verified; `v0.2.18` has no artifacts yet and its embedded manifests remain fail-closed with zero digests |
| Desktop | macOS Apple-silicon DMG, Windows installer, Linux AppImage; signing status attached; `DESKTOP_SIGNING` matches the attached status | `v0.2.18` is not tagged; the Apple notary credential must be restored before public release. Windows and Linux are explicitly unsigned; Intel is not part of this release |
...
| Publication | public repository, source tag equals artifact version, checksums and release notes | Keep `v0.2.16` private; tag `v0.2.18` only after source/CI review, then publish its reviewed assets and real manifest hashes before the production OTA |
