...
## Current audit evidence (2026-09-02, `v0.2.9` → `v0.2.20`)
...
profile is also unavailable. `v0.2.20` carries the Box parity fix and was published on 2026-09-02
...
| Publication | `v0.2.20` is public production (published 2026-09-02) with CLI binaries, checksums and digest-pinned images from the same tag. `v0.2.16` has public image tags and a private GitHub draft, but no public release. | Replace the unsigned `v0.2.20` DMG with a notarized build; hold the production OTA until the visible physical-device journey passes |
...
| Supply chain | versioned CLI + checksums; packaged Docker images pinned by digest | `v0.2.20` published CLI binaries with SHA-256 sidecars, an aggregate `checksums-0.2.20.txt`, and multi-architecture images pinned by digest in the release notes |
| Desktop | macOS Apple-silicon DMG, Windows installer, Linux AppImage; signing status attached; `DESKTOP_SIGNING` matches the attached status | `v0.2.20` shipped all three installers with `signing-status-*.json` attached and every platform explicitly unsigned; the Apple notary credential must be restored to replace the DMG. Intel is not part of this release |
...
| Publication | public repository, source tag equals artifact version, checksums and release notes | `v0.2.20` is tagged and public with checksums and release notes; keep `v0.2.16` private and hold the production OTA until the physical-device journey passes |
