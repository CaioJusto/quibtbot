...
| Desktop | macOS Apple-silicon DMG, Windows installer, Linux AppImage; signing status attached; `DESKTOP_SIGNING` matches the attached status | `v0.2.15`: Mac DMG remains pending until the notarized replacement is uploaded; Windows and Linux are explicitly unsigned; Intel is not part of this release |
...
| Publication | public repository, source tag equals artifact version, checksums and release notes | Publish the tag-generated assets and replace the embedded provisional CLI digests with the real v0.2.15 checksums before the production OTA |
