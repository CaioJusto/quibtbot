...
<p align="center">
  ...
  <a href="https://github.com/CaioJusto/quibtbot/releases/latest"><img src="https://img.shields.io/badge/release-v0.2.14-brightgreen.svg" alt="v0.2.14" /></a>
</p>
...
> **Where this is:** `v0.2.14` is published and running. What that means concretely: the three
...
>
> What is **not** covered yet: the `v0.2.14` desktop installers are the CI builds, **unsigned** on
> every platform. The Apple-silicon `QuibtBot.dmg` attached to `v0.2.14` is not notarized
...
> maintainer replaced the CI DMG by the notarized one); the same swap is pending for `v0.2.14`.
...
| --- | --- | --- |
| macOS (Apple silicon) | [`QuibtBot.dmg`](https://github.com/CaioJusto/quibtbot/releases/download/v0.2.14/QuibtBot.dmg) | CI build, unsigned and not notarized: macOS warns the first time — **right-click → Open**. |
| macOS (Intel) | — | No installer yet: run from source (`pnpm dev` + `pnpm desktop`). |
| Windows (64-bit) | [`QuibtBot-setup.exe`](https://github.com/CaioJusto/quibtbot/releases/download/v0.2.14/QuibtBot-setup.exe) | Unsigned test installer: SmartScreen warns (**More info → Run anyway**), and you install Docker Desktop yourself. |
| Linux (x64) | [`QuibtBot.AppImage`](https://github.com/CaioJusto/quibtbot/releases/download/v0.2.14/QuibtBot.AppImage) | Unsigned test AppImage: needs `libfuse2`, mark it executable and run it; you install Docker (Engine or Desktop) yourself. |
...
The `v0.2.14` release publishes those three desktop installers, standalone macOS/Linux/Windows
...
- The Compose stack runs as production and boots fail-closed: a secret that is missing, under 32
  characters, or still starting with `replace-with-` stops the service instead of running with a
  published example. The source Compose also wants `SANDBOX_SUPERVISOR_TOKEN`, `BOOTSTRAP_SECRET`,
  and either `RESEND_API_KEY` or `AUTH_EMAIL_DISABLED=true` — see
  [`docs/self-host.md`](docs/self-host.md). `pnpm dev` is unchanged.
- Local auto-login is loopback-only: on a LAN or public install `POST /api/local/session` answers
  `404` and entry is by password or pairing code. The desktop app signs itself in by proving it
  holds the local `quibt.env` secret (one minute, single use), not by its address; with no such
  file it falls back to the normal sign-in.
- Keys are separated by job. `BETTER_AUTH_SECRET` signs the session cookie only; the bot screen,
  the internal web→API proxy and the desktop capability use keys derived from it under their own
  labels.
- `models.connect` only probes an explicit local target (`127.0.0.1`, `localhost`,
