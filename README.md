...
[`docs/desktop.md`](docs/desktop.md#tray--menu-bar). The desktop app can also open a per-bot
embedded browser (not the host Chrome profile); web and mobile keep the Linux noVNC screen.

| Platform | Installer | One sentence |
| --- | --- | --- |
| macOS (Apple silicon) | [`QuibtBot.dmg`](https://github.com/CaioJusto/quibtbot/releases/latest/download/QuibtBot.dmg) | Check `signing-status-mac.json`; if it says the build is unsigned, use **right-click → Open**. |
| macOS (Intel) | — | No installer yet: run from source (`pnpm dev` + `pnpm desktop`). |
| Windows (64-bit) | [`QuibtBot-setup.exe`](https://github.com/CaioJusto/quibtbot/releases/latest/download/QuibtBot-setup.exe) | Unsigned test installer: SmartScreen warns (**More info → Run anyway**), and you install Docker Desktop yourself. |
| Linux (x64) | [`QuibtBot.AppImage`](https://github.com/CaioJusto/quibtbot/releases/latest/download/QuibtBot.AppImage) | Unsigned test AppImage: needs `libfuse2`, mark it executable and run it; you install Docker (Engine or Desktop) yourself. |
